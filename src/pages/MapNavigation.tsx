...
import {
  ...
  ChevronUp, ChevronDown, LocateFixed, Layers, ArrowLeftRight,
...
const osrmProfile: Record<TravelMode, string> = {
  ...
  walking: "foot",
  cycling: "bike",
  transit: "car", // OSRM has no transit; fallback to car
};
...
const MapNavigation = () => {
    ...
    L.marker([BUSINESS.lat, BUSINESS.lng], { icon: businessIcon })
      ...
      .bindPopup(`
        <div style="font-family:system-ui;min-width:180px">
          ...
          <span style="font-size:12px;color:#666">${BUSINESS.address}</span><br/>
          <span style="font-size:12px;color:#666">${BUSINESS.phone}</span>
        </div>
      `);
  ...
  useEffect(() => {
    ...
    if (!map) return;
    const osm = (map as any)._osmLayer as L.TileLayer;
    const sat = (map as any)._satLayer as L.TileLayer;
    if (satelliteView) { map.removeLayer(osm); sat.addTo(map); }
  ...
  const recenter = () => {
    if (userLocation && mapInstance.current) {
      ...
      mapInstance.current.flyTo([BUSINESS.lat, BUSINESS.lng], 13);
    }
  };
  ...
  const handleStepClick = (idx: number) => {
    setActiveStep(idx);
    if (routeInfo && mapInstance.current && routeInfo.geometry[idx]) {
      mapInstance.current.flyTo(routeInfo.geometry[Math.min(idx * 5, routeInfo.geometry.length - 1)], 16);
    }
  };
